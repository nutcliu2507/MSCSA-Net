...
def matrix(preddataset_path, truthdataset_path, class_Num):
    ...
    for i in range(len(pred_names)):
        ...
        pred[pred<0.5]= 0
        pred[pred>=0.5]= 1
        flag1= np.zeros((constants.input_shape[0],constants.input_shape[1]))
        for m in range(constants.input_shape[0]):
            for k in range(constants.input_shape[1]):
                if (pred[m,k,0]==1 and pred[m,k,1]==1 and pred[m,k,2]==1): flag1[m,k]= 0
                elif (pred[m,k,0]==0 and pred[m,k,1]==0 and pred[m,k,2]==1): flag1[m,k]=1
                elif (pred[m,k,0]==0 and pred[m,k,1]==1 and pred[m,k,2]==1): flag1[m,k]=2
                elif (pred[m,k,0]==0 and pred[m,k,1]==1 and pred[m,k,2]==0): flag1[m,k]=3
                elif (pred[m,k,0]==1 and pred[m,k,1]==1 and pred[m,k,2]==0): flag1[m,k]=4
                elif (pred[m,k,0]==1 and pred[m,k,1]==0 and pred[m,k,2]==0): flag1[m,k]=5
        
        
        
        truth = cv2.imread(truthdataset_path + '/' + truth_names[i])
        truth= cv2.cvtColor(truth, cv2.COLOR_BGR2RGB)
        truth=cv2.resize(truth, (256, 256), interpolation = cv2.INTER_AREA)
        truth = truth/255.0
        truth[truth<0.5]= 0
        truth[truth>=0.5]= 1
        
        flag2= np.zeros((constants.input_shape[0],constants.input_shape[1]))
        for m in range(constants.input_shape[0]):
            for k in range(constants.input_shape[1]):
                if (truth[m,k,0]==1 and truth[m,k,1]==1 and truth[m,k,2]==1): flag2[m,k]= 0
                elif (truth[m,k,0]==0 and truth[m,k,1]==0 and truth[m,k,2]==1): flag2[m,k]=1
                elif (truth[m,k,0]==0 and truth[m,k,1]==1 and truth[m,k,2]==1): flag2[m,k]=2
                elif (truth[m,k,0]==0 and truth[m,k,1]==1 and truth[m,k,2]==0): flag2[m,k]=3
                elif (truth[m,k,0]==1 and truth[m,k,1]==1 and truth[m,k,2]==0): flag2[m,k]=4
                elif (truth[m,k,0]==1 and truth[m,k,1]==0 and truth[m,k,2]==0): flag2[m,k]=5
...
#        truth[truth == 255] = 0
        ...
        # truth[truth == 8] = 3
        # truth[truth == 9] = 4
        matrix1 += hist(flag1, flag2, class_Num)
...
#    acc_overall = (TP+TN)/(FP+FN+TP+TN+1e-8)
    # Class accuracy
